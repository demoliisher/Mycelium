...
"""
Gitee platform module (sower side): push the encrypted feed into a Gitee repository.
...
- ``fork`` — disguise mode: fork a *Gitee* source repository into this
  account under the **same name** (the Gitee fork API does not support
...
Cross-platform sources (GitHub, GitLab, ...) are **not** supported yet:
the Gitee OpenAPI v5 has no import/clone endpoint, so such fork links are
rejected with a hint to import the repository manually on the Gitee web UI.

Gitee materializes forks asynchronously: a write issued right after
forking can transiently fail with HTTP 400 "文件新建失败" (file creation
failed); ``push`` retries such errors briefly.
...
__all__ = ["GiteeClient"]
# Note: the Gitee API can only create private repositories; making a
# repository public requires applying manually on the Gitee web console.


class GiteeClient(GitPlatformClient):
    """
    Gitee API v5 client bound to a single repository (sower side).
    ...
    - ``fork`` mode: the target is ``<namespace>/<source name>`` — the fork
      keeps the source's name, since the Gitee fork API cannot rename.
    ...
    def _split_fork(cls, fork: str) -> tuple[str, str]:
        """
        Parse a Gitee fork source address into (owner, repo).
        ...
        Raises:
            ValueError: malformed address, or a non-Gitee source.
                Cross-platform forks are not supported yet — the Gitee API
                cannot import them; import the repository manually on the
                Gitee web UI instead.
        """
        ...
        if "@" in src or (host is not None and host not in cls._FORK_HOSTS):
            raise ValueError(
                "cross-platform forks are not supported yet: the Gitee API "
                "cannot import them; import the repository manually on the "
                f"Gitee web UI instead (got {fork!r})"
            )
        parts = [p for p in src.split("/") if p]
        if len(parts) < 2:
            raise ValueError(
                f"fork must look like 'owner/repo' or a full Gitee URL, got {fork!r}"
            )
    ...
    def ensure_repo_exists(self) -> bool:
        ...
        - fork mode: reuse an existing same-named repository, otherwise fork
          the Gitee source under the same name.
        """
    ...
    def fork_repo(self, fork: str, organization: str | None = None) -> dict:
        """
        Fork a *Gitee* repository into this account under its original name.

        The Gitee fork API does not support custom names, so the fork always
        keeps the source's name (the constructor already resolved the target
        to ``<namespace>/<source name>``). If the API reports a different
        path it is renamed defensively.
        """
    ...
    def push(
        ...
        """
        Create or update a file in the repository; return Gitee's JSON response.
    ...
    def _write_file(
        ...
        """
        Gitee splits "create" and "update" into two endpoints:
        - file missing → POST /contents/{path} (no sha needed)
    ...
    def _is_fork_race(self, exc: requests.exceptions.HTTPError) -> bool:
        """True if the error is Gitee's transient fork-materialization race.

        The fork record appears to the API quickly, but the branch refs are
        still being initialized, so a write issued right after forking can
        transiently fail with HTTP 400 "文件新建失败" (file creation failed).
        """
        if exc.response is None or exc.response.status_code != 400:
            return False
        return "文件新建失败" in (exc.response.text or "")
    ...
    def _existing_sha(self, endpoint: str) -> str | None:
        ...
        (Gitee); a non-empty list means the path is a directory.
        """
        try:
    ...
    def _git_identity(self) -> tuple[str, str] | None:
        ...
        Resolved from the Gitee profile (``GET /user``): the login and the
        profile email. When the profile is unreachable or lacks an email
        there is no fallback identity — returns None so ``push`` re-raises
    ...
    def _git_remote(self) -> tuple[str, str, str] | None:
        ...
        Gitee's HTTPS git endpoint accepts the account login with the
        access token as the password.
        """
        return (
            f"https://gitee.com/{self.namespace}/{self.repo}",
            self.namespace,
            self.access_token,
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        raise SystemExit(
            "usage: python -m mycelium.interface.sower.gitee <access_token>"
        )
    token = sys.argv[1]
    ...
    # user's profile — there is no owner parameter.
    client = GiteeClient(
        access_token=token,
        repo="my-feed-repo",
        branch="master",
    ...
    # Fork mode (disguise): fork a *Gitee* source into a same-named repo.
