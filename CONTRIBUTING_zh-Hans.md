...
模块文档统一放在 `docs/` 下，目录镜像包结构（`docs/crypto/`、`docs/interface/sower/`……）。英文 `README.md` 是**必须**的——它是事实来源。其他语言的翻译**可选**：欢迎添加或更新翻译（简体中文使用 `README_zh-Hans.md` 命名，其他语言沿用同样的 `<语言标签>` 后缀规则）。已有翻译版本时，须与英文原文保持同步。
